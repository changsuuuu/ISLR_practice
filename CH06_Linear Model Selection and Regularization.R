...
###
library(leaps)
regfit.full <- regsubsets(Salary ~ ., Hitters) #leaps에 있는 함수
...
### The which.max() function can be used to 
...
# Note that by default, the glmnet() function standardizes the variables 
...
# 근데 이렇게 해서 Lse 할때 exact=T 해줘여함 이유는 p.277
mean((ridge.pred - y.test)^2)
lm(y ~ x, subset = train)
